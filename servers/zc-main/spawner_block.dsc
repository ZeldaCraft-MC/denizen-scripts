spawner_events:
  ...
  events:
    ...
    on entity prespawns because spawner in:s1:
      - if <server.has_flag[spawners_disabled]> || !<context.spawner_location.center.find.blocks[spawner].within[2.1].remove[1].is_empty>:
        - determine cancelled
    ...
    on player places spawner:
      - if !<context.location.center.find.blocks[spawner].within[2.1].remove[1].is_empty>:
