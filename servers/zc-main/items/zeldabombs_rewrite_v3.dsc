zeldabombv3:
    ...
    events:
        ...
        - determine cancelled
        #todo: shift clicking and re-pickup water bombs causes them to sink
        #todo: very strange behavior when thrown while swimming on waterbed
        ...
        - if <context.item.script.name> != water_bomb_item && ( <player.eye_location.is_liquid> || <player.eye_location.material.waterlogged||false> || <player.eye_location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ):
            - stop
        - else if <player.has_flag[lifting]||false> || <player.has_passenger>:
            - if <player.flag[lifting].as_entity.scriptname> == BOMB_ENTITY:
                - define held_bomb <player.flag[lifting].as_entity>
                - flag <[held_bomb]> mount_cancel
                - mount cancel <[held_bomb]>
                - flag <[held_bomb]> liftable
                - wait 1t
                - if <player.is_sneaking> && ( <player.cursor_on[5].material.is_solid||false> && <player.cursor_on[5].above.material.name||none> == air ) || ( <[held_bomb].has_flag[water_bomb]> && ( <player.cursor_on[5].above.is_liquid> || <player.cursor_on[5].above.material.waterlogged||false> || <player.cursor_on[5].above.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ) ) :
        	...
        	- else:
                ...
                - stop
        - else if <player.has_flag[bombcooldown]> || <player.cursor_on[5].has_inventory||false> || <player.cursor_on[5].material.contains_any[<list[button|lever|door|sign]>]||false> || <player.is_inside_vehicle>:
            - stop
        - else if <player.is_sneaking>  && !<player.has_flag[lifting]||false> && ( <context.location.material.is_solid||false> && <context.location.above.material.name||none> == air ) || ( <context.item.script.name> == water_bomb_item && ( <context.location.above.is_liquid> || <context.location.above.material.waterlogged||false> || <context.location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ) ) :
            - if <player.gamemode> != CREATIVE:
                - take bomb_item quantity:1 from:<player.inventory>
            ...
            #todo: should this be enabled, or just cancelled for water bombs?
        ...
        - if <context.item.lore.contains[NoFuse]||false>:
            - stop
        - run prime d:<[spawned_bomb]>|<player>|<context.item.nbt[fuse_time]||duration[8s]> instantly
        ...
        - if <player.flag[lifting]||false> == <context.entity> && <context.entity.flag[player_lifting]||false> == <player>:
            ...
            - if <player.is_sneaking> && ( <player.cursor_on[5].material.is_solid||false> && <player.cursor_on[5].above.material.name||none> == air ) || ( <context.entity.has_flag[water_bomb]> && ( <player.cursor_on[5].above.is_liquid> || <player.cursor_on[5].above.material.waterlogged||false> || <player.cursor_on[5].above.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ) ) :
                ...
                - wait 5t
                #todo should this be removed?? or just not triggered for water bombs
                #- run onland def:<context.entity>
            - else:
                - adjust <context.entity> equipment:air|air|air|bomb_item_model[custom_model_data=<context.entity.flag[model_data]||100>]
                - adjust <context.entity> item_in_hand:air
                - if <context.entity.has_flag[water_bomb]> && ( <player.eye_location.is_liquid> || <player.eye_location.material.waterlogged||false> || <player.eye_location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ):
        ...
        - if <player.has_flag[lifting]>:
            - define bomb <player.flag[lifting].as_entity>
        ...
        - define owner <context.damager.flag[spawned_by].as_player.name||someone>
...
prime:
    ...
    debug: false
    script:
    - define explode_time <util.time_now.add[<[3].as_duration>]||util.time_now.add[8s]>
    - define bomb_entity <[1].as_entity>
    - run fuse_sfx def:<[bomb_entity]>|<[explode_time].from_now.in_seconds>
    - adjust <[bomb_entity]> custom_name:<[3].as_duration>
    ...
    - while <[bomb_entity].is_spawned> && <util.time_now.is_before[<[explode_time].sub[1s]>]>:
        - adjust <[bomb_entity]> custom_name:<[3].as_duration.sub[<util.time_now.duration_since[<[prime_start_time]>]>].formatted.before[.].before[s]>
        - if <[bomb_entity].has_flag[player_lifting]>:
            - title targets:<[bomb_entity].flag[player_lifting]> "subtitle:<red>Bomb exploding in: <[3].as_duration.sub[<util.time_now.duration_since[<[prime_start_time]>]>].formatted.before[.].before[s]> seconds" stay:8t fade_in:0t fade_out:0t
        - playeffect <[1].location.add[0,0.2,0]> effect:smoke visibility:20 quantity:10 offset:0.05,0.3,0.05
        - if !<[bomb_entity].has_flag[water_bomb]> && ( ( <[bomb_entity].location.is_liquid> && !<[bomb_entity].location.material.contains[bubble_column]> ) || <[bomb_entity].location.material.waterlogged||false> || <[bomb_entity].location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ):
        ...
        - else if !<util.time_now.is_before[<[explode_time].sub[5s]>]> && <[flashing]> < 1:
            ...
            - define flashing 1
        - wait 5t
    - if <server.entity_is_spawned[<[1]>]||false> :
...
onland:
    ...
    - if <[bomb].has_flag[water_bomb]>:
        - while <[bomb].is_spawned||false> && ( <[bomb].location.is_liquid||false> || <[bomb].location.material.waterlogged||false> || <[bomb].location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]||false> ):
    ...
    - if <context.item.lore.contains[NoFuse]||false>:
        - if <[1].location.is_liquid||false> || <[1].location.material.waterlogged||false> || <[1].location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]||false>:
            #- adjust <[1]> armor_pose:head|3.15,0,0
            - wait 1s
            - while ( <[1].location.is_liquid> || <[1].location.material.waterlogged||false> || <[1].location.material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ) && <server.entity_is_spawned[<[1]>]||false>:
...
bomb_explode:
    ...
    debug: false
    script:
    - define loc <[1].as_entity.location>
    - define bomb <[1].as_entity>
    - flag <[bomb]> exploded
    ...
    - if ( <[loc].is_liquid> || <[loc].material.waterlogged||false> || <[loc].material.contains_any[<list[kelp|kelp_plant|seagrass|tall_seagrass]>]> ) && !<[loc].material.contains[lava]> && !<[bomb].has_flag[water_bomb]> :
    ...
    - else :
        ...
        - hurt 10 <[loc].find.entities.within[5].exclude[bomb_entity]||null> cause:ENTITY_EXPLOSION source:<[bomb]>
        - foreach <[loc].find.entities[ARMOR_STAND|SHULKER].within[5].exclude[<[bomb]>]||li@> :
            ...
            - if <[value].has_flag[bomb_entity]> && <[value].is_spawned>:
                ...
                - wait 2t
        - foreach <server.flag[bombable_blocks].shared_contents[<[loc].find.blocks.within[5]||li@>]||li@>:
        ...
        - foreach <server.flag[temp_bombable_blocks].shared_contents[<[loc].find.blocks.within[5]||li@>]||li@>:
            - modifyblock <[value]> air
            - playeffect <[value]> effect:campfire_cosy_smoke quantity:1 offset:0,0.5,0 data:0.01
