  ...
  events:
    on player enters bomb_demo_loc:
      - foreach <context.area.entities[armor_stand]>:
        - if <[value].has_flag[spawn_bombs]> && <[value].equipment.get[4].custom_model_data||false> == 100:
      ...
      - wait 1.2s
      - foreach <context.area.entities[armor_stand]>:
        - if <[value].has_flag[spawn_bombs]> && <[value].equipment.get[4].custom_model_data||false> == 300:
