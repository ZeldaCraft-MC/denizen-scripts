...
voting_prizes:
  ...
  prizes:
    rare:
      ...
      - paper[custom_model_data=16102;display_name=Xmas 2022 Bayli Plushie]
      - paper[custom_model_data=16103;display_name=Xmas 2022 Gingeh Plushie]
      - paper[custom_model_data=16104;display_name=Xmas 2022 Isaiah Plushie]
  ...
  procedural items:
    ...
    - foreach <script[voting_sites].data_key[sites].keys> as:site:
      - if !<player.has_flag[voted_<[site]>]>:
        - define item "<item[voting_link_i].with_single[display=<&e>Vote for <&a><[site].parsed||error>]>"
        - define item "<[item].with_single[lore=<&e>Voting Link<&co>|<&2><script[voting_sites].data_key[sites].get[<[site]>].parsed||error>]>"
        - define items:->:<[item].with_single[flag=voting_site:<[site].parsed||error>]>
      - else:
        - define item "<item[cant_vote_i].with_single[display=<&c>Cannot vote on <[site].parsed||error>]>"
        - define lore_exp "<&f><player.flag_expiration[voted_<[site].parsed||error>].from_now.formatted||error> until you can vote again"
        - define "items:->:<[item].with_single[lore=<&f>Looks like you already voted today|<[lore_exp]>]>"
    - determine <[items]>
...
voting_world:
  ...
  events:
    ...
    - if <[player]> == nobody:
      - ~discordmessage id:zc-info channel:859064390888587284 "<discord_embed.with[title].as[Vote has nobody].with[description].as[Username: <context.username><&nl>Service: <context.service>].with[color].as[red].with[timestamp].as[<util.time_now>]>"
      - stop
    - if <[player].has_flag[voted_<context.service>]>:
      - ~discordmessage id:zc-info channel:859064390888587284 "<discord_embed.with[title].as[Player voted already].with[description].as[Username: <context.username><&nl>Service: <context.service>].with[color].as[red].with[timestamp].as[<util.time_now>]>"
      - stop
    - if !<script[voting_sites].data_key[sites].keys.contains[<context.service>]>:
      - ~discordmessage id:zc-info channel:859064390888587284 "<discord_embed.with[title].as[Incorrect voting site].with[description].as[Username: <context.username><&nl>Service: <context.service>].with[color].as[red].with[timestamp].as[<util.time_now>]>"
      - stop
    ...
    - if !<[player].inventory.can_fit[rare_vote_key]> || !<[player].inventory.can_fit[common_vote_key]>:
      - flag <[player]> vote_prizes:++
      - ~discordmessage id:zc-info channel:859064390888587284 "<discord_embed.with[title].as[Players inventory was full].with[description].as[Username: <context.username><&nl>Service: <context.service><&nl>UUID: <[player].uuid><&nl>saved votes: <[player].flag[vote_prizes]>].with[color].as[yellow].with[timestamp].as[<util.time_now>]>"
...
votifier_alive_check:
  ...
  events:
    on votifier vote:
    - announce "<context.username> voted on <context.service> at <util.time_now>" to_console
...
bottled_fart_lol:
  ...
  debug: false
  events:
    after player right clicks block with:bottled_fart:
      - ratelimit <player> 2t
