  ...
  script:
    ...
    - if <context.args.get[1].contains_text[.]> || <context.args.get[1].contains_text[:]>:
      ...
      - if <server.is_banned[<context.args.get[1]>]>:
        - define color <&c>
        - define exp_dur <server.ban_info[<context.args.get[1]>].expiration||null>
        - if <[exp_dur]> == null:
          - define exp <&c>permanently
        - else:
          - define curr_time <util.time_now>
          - define time <[curr_time].add[<[exp_dur].in_milliseconds.sub[<[curr_time].epoch_millis>].div[1000]>]>
          - define exp "until <&c><[time].format[MMM d yyyy h:mm a z]> (<[time].from_now.formatted> remaining)"
